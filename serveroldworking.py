# RetroFuse OPS Dashboard server.py (v1.7_full_restore_ascii)
...
APP_VERSION = "v1.7_full_restore_ascii"
...
def _list_browser_procs() -> List[Dict[str, Any]]:
    ...
    """
    ps = r"""
$names = @('chrome.exe','vivaldi.exe','msedgewebview2.exe')
...
def _detect_vivaldi_rc3(procs: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Optional RC3 detection: any vivaldi.exe.
    """
    for pr in procs:
        if (pr.get("name") or "").lower() == "vivaldi.exe":
            return {
                "status": "running",
                "process_id": pr.get("pid"),
...
def _build_latest_snapshot() -> Dict[str, Any]:
    ...
    # Keep existing rc2/rc3 if present, but ensure keys exist
    if not isinstance(snap.get("rc2"), dict):
        snap["rc2"] = {"status": "unknown", "detectable": False, "expected": {"note": "RC2 not yet distinct; detection pending"}}
    else:
        snap["rc2"].setdefault("status", "unknown")
        snap["rc2"].setdefault("detectable", False)
        snap["rc2"].setdefault("expected", {"note": "RC2 not yet distinct; detection pending"})
    ...
    <header class="header">
      <div>
        <div class="title">
          ...
          <span class="title-pill">OPS SPINE - v1.7 - TELEMETRY</span>
        </div>
...
<script>
  ...
  function statusClass(s) {
    ...
    if (v === "not_running") return "value-warn";
    return "value-bad";
  }
